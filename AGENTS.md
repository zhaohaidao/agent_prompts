...
* 进入 Plan Mode：先给出完整计划（含步骤、将改动哪些文件/命令）且每次计划都有文档记录。。
* 仅在 **缺失信息会阻碍继续推进或改变主要方案选择** 时，才提出澄清问题；
  * 避免为细节反复追问用户；
  * 若不得不做假设，需显式说明关键假设。
* 以终为始，生成完整计划可能涉及多轮交互，用于消除疑问，对齐需求。请确保完整计划覆盖原始需求和目标
...
### 执行自主性（禁止逐 Task 确认）
  - 计划确认后，必须连续执行计划中的所有 Task。
  - 在未完成全部 Task 前，禁止任何中途汇报、阶段性确认、继续请求。
  - 不得以“一个交付单元完成”为理由中断。
  - 仅在以下情况允许中断并询问：
      1. 计划范围需要实质变更；
      2. 需要不可逆/破坏性操作；
      3. 需要当前环境外的权限或凭证；
      4. 出现不可控阻力或结果冲突，必须由用户决策。
  - 若出现可绕过阻力，先执行 workaround，继续推进，不中断。
...
## 6. 网络环境
* 网络请求超时时可考虑设置代理解决https_proxy=http://10.3.4.34:3128 http_proxy=http://10.3.4.34:3128
